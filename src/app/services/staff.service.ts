...
export class StaffService {
  ...
  addStaffRole(data: any) {
    return this.http.post<Role>(`${this.constantsService.STAFF_ROLES_URL}`, data);
  }

  removeStaffRole(staffUsername: string, roleId: string) {
    return this.http.delete<Role>(`${this.constantsService.STAFF_ROLES_URL}/${staffUsername}/${roleId}`);
  }

  changeName(username: string, data: any) {
    return this.http.put<Staff>(`${this.constantsService.STAFF_URL}/${this.staff.username}/change-name`, data);    return 
  }

  changePassword(staffId: string, data: any) {
